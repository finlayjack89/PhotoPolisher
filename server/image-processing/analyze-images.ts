...
export async function analyzeImages(req: AnalyzeImagesRequest) {
  ...
  if (!GEMINI_API_KEY) {
    ...
    throw new Error('GEMINI_API_KEY is not configured. Please add your Gemini API key to continue.');
  }
  ...
  for (const file of files) {
    try {
      ...
      // Defensively extract text from response
      const candidate = data.candidates[0];
      if (!candidate?.content?.parts) {
        throw new Error('Invalid Gemini response structure');
      }
      
      // Find first text part (skip inline_data or other types)
...
export async function analyzeBackdrop(req: AnalyzeBackdropRequest) {
  ...
  try {
    ...
    // Defensively extract text (same pattern as analyzeImages)
    const candidate = payload?.candidates?.[0];
    if (!candidate?.content?.parts) {
      console.warn('Invalid Gemini response structure for backdrop analysis');
