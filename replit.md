...
- **Fixed Double-Opacity Bug**: Corrected opacity application to prevent invisible reflections (6% visibility issue). Opacity is now baked into reflection texture during generation via destination-in compositing, then drawn to main canvas with `ctx.globalAlpha = 1.0` to avoid double-fading. This ensures the configured opacity (e.g., 0.25) is applied once, not multiplied twice (0.25 × 0.25 = 0.0625).
- **Smart Gap Fix for Transparent Padding**: Implemented `getBottomPadding()` utility to detect and eliminate the "double-gap" issue caused by invisible transparent padding in source images. When images are flipped for reflection, bottom padding becomes top padding, creating a visible gap. The fix measures raw padding via bottom-up alpha channel scan (threshold >10), scales it to current draw size (visualPadding = rawPadding × scaleFactor), and calculates reflectionY = productY + height - (visualPadding × 2) - 2 to "pinch out" the combined empty space. This ensures pixel-to-pixel contact between product and reflection regardless of source image padding. Both compositeLayersV2 and legacy compositeLayers use identical algorithms with 📏 [Smart Gap] debug logging. Reflections now respect user placement strictly and crop naturally at canvas edges (photometrically correct) without aggressive auto-lift positioning.
- **Contact Shadow (Ambient Occlusion) System**: Adds realistic grounding effect to make products appear anchored to the backdrop surface. The `generateContactShadow()` function creates a downscaled (max 500px) black silhouette using source-in compositing for performance. Contact shadows are squashed to 15% of product height, positioned tucked underneath (shadowY = productY + height - shadowH × 0.6), and rendered with blur(8px) + 40% opacity for soft, natural appearance. Implemented only in `compositeLayersV2` with full Safe Mode protection: outer try/catch handles generation failures, inner try/finally guarantees ctx.restore() and cleanupCanvas() always execute to prevent blur filter leakage to product layer. Proper z-ordering: Backdrop → Cloudinary Shadow → Reflection → Contact Shadow → Product. Debug logging with 🌑 [Contact Shadow] prefix confirms successful application.
