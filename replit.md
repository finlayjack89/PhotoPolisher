...
- **Auto-Lift Canvas Clipping Prevention**: Intelligent auto-lift logic prevents reflections from being drawn off-screen when products are positioned near canvas bottom. Calculates visualBottom (productY + productHeight + reflectionHeight - 2px overlap) before drawing, detects overflow beyond canvas.height, and shifts entire composition (shadow + product + reflection) upward by overflow + 20px safety margin. Both compositeLayersV2 and legacy compositeLayers use identical geometry calculations ensuring lift amount matches actual render dimensions (reflection = 60% of product height). Debug logging with 🚀 [Auto-Lift] prefix shows lift amount, visualBottom, and canvasHeight for troubleshooting.
